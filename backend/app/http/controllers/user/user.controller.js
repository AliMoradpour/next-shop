...
const { checkOtpSchema, completeProfileSchema, updateProfileSchema } = require("../../validators/user/user.schema");
...
class userAuthController extends Controller {
  ...
  async getOtp(req, res) {
    ...
    if (!phoneNumber) throw createError.BadRequest("شماره موبایل معتبر را وارد کنید");
  ...
  async checkOtp(req, res) {
    ...
    const user = await UserModel.findOne({ phoneNumber }, { password: 0, refreshToken: 0, accessToken: 0 });
    // .populate([
    //   {
    //     path: "Products",
    //     model: "Product",
    ...
    if (user.otp.code != code) throw createError.BadRequest("کد ارسال شده صحیح نمیباشد");
    ...
    if (!user.isActive) WELLCOME_MESSAGE = `کد تایید شد، لطفا اطلاعات خود را تکمیل کنید`;
  ...
  async updateUser(phoneNumber, objectData = {}) {
    ...
    const updatedResult = await UserModel.updateOne({ phoneNumber }, { $set: objectData });
  ...
  async completeProfile(req, res) {
    ...
    if (!user.isVerifiedPhoneNumber) throw createError.Forbidden("شماره موبایل خود را تایید کنید.");

    const duplicateUser = await UserModel.findOne({ email });

    if (duplicateUser) throw createError.BadRequest("کاربری با این ایمیل قبلا ثبت نام کرده است.");
